...
static uint32_t rpi3_gpio_base_address;
...
static int
rpi3_gpio_claim(char *owner, int nr, struct gpio **gpio)
{
...
static int
rpi3_gpio_pin_mode(struct gpio *gpio, int mode)
{
	assert(gpio != NULL);
	gpio->mode = mode;
	...
	set32(rpi3_gpio_base_address + 4*gpio_reg, 0x7 << gpio_offset, mode << gpio_offset);

	return 0;
}

static int
rpi3_gpio_set(struct gpio *gpio, int value)
{
	assert(gpio != NULL);
	...
	if (value == 1) {
		write32(rpi3_gpio_base_address + 0x1c + 4*gpio_reg,
		    BIT(gpio->nr % 32));
	} else {
		write32(rpi3_gpio_base_address + 0x28 + 4*gpio_reg,
		    BIT(gpio->nr % 32));
	}
	return OK;
}
...
static int
rpi3_gpio_read(struct gpio *gpio, int *value)
{
	assert(gpio != NULL);
	assert(gpio->nr >= 0 && gpio->nr <= 53);
	...
	*value = (read32(rpi3_gpio_base_address + 0x34 + 4*gpio_reg) >>
	          (gpio->nr % 32)) & 0x1;
...
static int
rpi3_gpio_intr_read(struct gpio *gpio, int *value)
{
	/* No interrupts yet */
	return OK;
}
...
static int
rpi3_gpio_init(struct gpio_driver *gpdrv)
{
	struct minix_mem_range mr;

	mr.mr_base = PADCONF_RPI3_REGISTERS_BASE;
	mr.mr_limit = mr.mr_base + PADCONF_RPI3_REGISTERS_SIZE;
	...
	rpi3_gpio_base_address =
		(uint32_t) vm_map_phys(SELF,
		(void *) PADCONF_RPI3_REGISTERS_BASE, PADCONF_RPI3_REGISTERS_SIZE);

	if (rpi3_gpio_base_address == (uint32_t) MAP_FAILED) {
		log_warn(&log, "Unable to map GPIO memory\n");
		return EPERM;	/* fixme */
	}

	gpdrv->claim = rpi3_gpio_claim;
	gpdrv->pin_mode = rpi3_gpio_pin_mode;
	gpdrv->set = rpi3_gpio_set;
	gpdrv->read = rpi3_gpio_read;
	gpdrv->intr_read = rpi3_gpio_intr_read;
	gpdrv->message_hook = rpi3_message_hook;
	return 0;
}
